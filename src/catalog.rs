...
impl VoiceEntry {
    pub fn display_name(&self) -> String {
        if self.key == FEATURED_VOICE {
            return format!("English Voice Collection · {} voices", self.num_speakers);
        }
    ...
    pub fn matches_search(&self, query: &str) -> bool {
        ...
        let friendly_terms = if self.key == FEATURED_VOICE {
            "recommended best variety collection many lots hundreds 904 offline"
        } else {
            ""
        };
        let haystack = format!(
            ...
            self.name.replace('_', " "),
            self.language.name_english,
            self.language.name_native,
            self.language.country_english,
            self.quality.replace('_', " "),
        )
        .to_lowercase();
...
mod tests {
    ...
    #[test]
    fn featured_collection_matches_friendly_search_terms() {
        let catalog = VoiceCatalog::parse(include_str!("../assets/voices.json"))
            .expect("bundled catalog should parse");
        let voice = catalog
            .get(FEATURED_VOICE)
            .expect("featured voice should exist");

        assert!(voice.matches_search("English Voice Collection"));
        assert!(voice.matches_search("904"));
        assert!(voice.matches_search("many voices"));
        assert!(voice.matches_search("recommended variety"));
        assert!(!voice.matches_search("Japanese"));
    }
}
