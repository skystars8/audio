        ...
        for event in events {
            match event {
                ...
                JobEvent::VoiceInstalled(key) => {
                    self.reload_installed();
                    self.preferences.selected_voice = key;
                    self.status = "Voice pack installed and ready offline".to_owned();
                }
            ...
            .show(root_ui, |ui| {
                ...
                ui.add(
                    egui::TextEdit::singleline(&mut self.preferences.search)
                        .hint_text("Search voices, languages, accents…"),
                );
                ui.horizontal(|ui| {
                ...
                let mut visible: Vec<VoiceEntry> = self
                    ...
                    .filter(|entry| {
                        ...
                        let haystack = format!(
                            ...
                            entry.key,
                            entry.name,
                            entry.language.name_english,
                            entry.language.country_english,
                            entry.quality
                        )
                        .to_lowercase();
                        language_matches
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if installed_voice.is_some() {
                            ui.label(egui::RichText::new("● Offline").color(egui::Color32::from_rgb(74, 180, 120)));
                        } else if ui
                            .add_enabled(!self.busy, egui::Button::new("Install voice pack"))
                            .clicked()
                        {
                ...
                if let Some(installed_voice) = installed_voice {
                    ...
                    if installed_voice.num_speakers > 1 {
                        ...
                        let current_label = installed_voice
                            .speaker_id_map
                            .iter()
                            .find_map(|(label, id)| (*id == speaker_id).then_some(label.as_str()))
                            .map(str::to_owned)
                        ...
                        ui.horizontal(|ui| {
                            ...
                            ui.label(format!("of {} · {current_label}", installed_voice.num_speakers));
                        });
                        self.preferences
                            .speaker_ids
            ...
            egui::Window::new("Setup & storage")
                ...
                .show(context, |ui| {
                    ...
                    if let Some(engine) = &self.engine {
                        ...
                        ui.label(
                            egui::RichText::new(
                                "This performs a one-time download into an isolated user directory. No administrator access is needed.",
                            )
                            .weak(),
                        );
                    }
